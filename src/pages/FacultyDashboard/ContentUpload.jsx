...
// A 6 GB file = ~96 chunks. Each chunk goes through backend proxy to YouTube.
...
function VideoForm({ faculty }) {
  ...
  const uploadChunkXHR = (uploadUrl, chunk, start, totalSize) =>
    new Promise((resolve, reject) => {
      ...
      // Pass YouTube upload URL and range as headers
    ...
    new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status === 308) {
          ...
          reject(new Error(`Resume query failed (HTTP ${xhr.status})`));
        }
      };
  ...
  const uploadToYouTube = async (uploadUrl, videoFile, startOffset = 0) => {
    ...
    while (offset < totalSize) {
      ...
      try {
        ...
        // On network error: wait 5s, ask YouTube where we got to, retry
        if (abortRef.current) throw new Error("Upload cancelled");
      ...
      // 308 — chunk accepted, advance offset
  ...
  const handleResume = async () => {
    ...
    try {
      ...
      setError("Could not resume — session may have expired (sessions last 6 days). Start a new upload.");
      clearResumeState();
    }
  };
  ...
  const startUpload = async (existingUploadUrl, startOffset = 0) => {
    ...
    try {
      ...
      // Step 1 — Create session (only if not resuming)
      if (!uploadUrl) {
      ...
      // Step 2 — Upload via backend proxy
  ...
  return (
    <form className="space-y-4" onSubmit={handleUpload}>
      ...
      {resumeState && !loading && !success && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-sm space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={clearResumeState}
      ...
      <input placeholder="Subject" className="input" value={form.subject}
        disabled={loading} onChange={(e) => setForm({ ...form, subject: e.target.value })} />
      <input placeholder="Title" className="input" value={form.title}
        disabled={loading} onChange={(e) => setForm({ ...form, title: e.target.value })} />
      <input placeholder="Unit (e.g. Unit 1)" className="input" value={form.unit}
        disabled={loading} onChange={(e) => setForm({ ...form, unit: e.target.value })} />
      ...
      <div className="rounded-xl border-2 border-dashed border-gray-200 p-4">
        ...
        {file && (
          <p className="text-xs text-gray-500 mt-2">
            📹 {file.name} — {file.size > 1024 * 1024 * 1024
        ...
        <div className="mt-4">
          ...
          <div className="rounded-xl overflow-hidden border border-gray-200">
            <iframe src={embedUrl} width="100%" height="220"
              ...
              title="Video Preview" style={{ display: "block" }} />
          </div>
        </div>
      )}
    </form>
...
function PDFForm({ faculty }) {
  ...
  const [error, setError]     = useState("");
  const [success, setSuccess] = useState(false);
  const [form, setForm]       = useState({ subject: "", title: "", year: "", semester: "", unit: "" });
  ...
  const handleUpload = async (e) => {
    ...
    if (!form.subject || !form.title || !form.year || !form.unit ||
        (form.year !== "1" && !form.semester)) { setError("Please fill all fields"); return; }
...
function AssessmentForm({ faculty }) {
  ...
  const [error, setError]     = useState("");
  const [success, setSuccess] = useState(false);
  const [form, setForm]       = useState({ title: "", subject: "", year: "", semester: "", unit: "" });

  const handleDownloadTemplate = () => {
    const link = document.createElement("a");
    link.href = "/sample_mcq_template.xlsx"; link.download = "sample_mcq_template.xlsx";
  ...
  const handleUpload = async (e) => {
    ...
    if (!form.title || !form.subject || !form.year || !form.unit ||
        (form.year !== "1" && !form.semester)) { setError("Fill all fields"); return; }
  ...
  return (
    <form className="space-y-4" onSubmit={handleUpload}>
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-blue-600 mb-3 leading-relaxed">
            Fixed headers: <span className="font-mono bg-blue-100 px-1 rounded">Question | Option A | Option B | Option C | Option D | Answer</span>.
      ...
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
        ...
        <input id="assessment-file-input" type="file" accept=".xlsx,.csv,.docx,.txt"
          disabled={loading} onChange={(e) => setFile(e.target.files[0])}
          className="block w-full text-sm text-gray-500
            file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0
